...
int main(int argc, char *argv[]) {
    ...
    task.size = totalPermutations;
    task.write_runtime = 0;
    task.kernel_runtime = runtime_ms(kernel_evt);
    task.search_runtime =
            npass == 1 ? runtime_ms(search_evt[0]) : total_runtime_ms(search_evt[0], search_evt[npass - 1]);
    task.read_runtime = runtime_ms(read_evt);
...
//endregion
    ...
    FILE *f = fopen("v3.csv", "a");
    printResultV3(f, task);
    fclose(f);
