...
int main(int argc, char *argv[]) {
    ...
    while (continueLoop) {
        ...
        err = clEnqueueUnmapMemObject(info.queue, d_permutations, permutations, 1, &write_evt, &unmap_evt);
        ocl_check(err, "unmap costs");

        kernel_evt = kernel(
                info.queue, info.kernel, &unmap_evt, 1, info.preferred_multiple_init,
