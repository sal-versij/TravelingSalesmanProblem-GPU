...
int main(int argc, char *argv[]) {
    ...
    struct Info info = initialize("GlobalArray_SingleResult");
...
//    print_graph(adj, v);
    ...
    int *permutations = malloc(permutations_size);
    ...
    cl_mem d_costs = clCreateBuffer(info.context, CL_MEM_WRITE_ONLY | CL_MEM_ALLOC_HOST_PTR, costs_size, NULL, &err);
    ...
    while (continueLoop) {
        ...
        chunkRun.write_bw = current_number_of_permutations * (v - 1) * sizeof(int) / chunkRun.read_runtime / 1e6;
        chunkRun.kernel_bw = 2.0 * v * current_number_of_permutations * sizeof(int) / chunkRun.read_runtime / 1e6;
        chunkRun.read_bw = costs_size / chunkRun.total_runtime / 1e6;
        chunkRun.total_bw =
                (current_number_of_permutations * (v - 1) * sizeof(int) +
                 2.0 * v * current_number_of_permutations * sizeof(int) +
