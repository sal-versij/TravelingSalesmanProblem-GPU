...
cl_event kernel(cl_command_queue q, cl_kernel k, size_t lws, cl_mem d_adj, cl_mem d_costs, cl_int v,
                cl_ulong work_size) {
    cl_int err;
    int i = 0;
    ...
    cl_event kernel_evt;
    err = clEnqueueNDRangeKernel(q, k, 1, NULL, &lws, &lws, 0, NULL, &kernel_evt);
...
int main(int argc, char *argv[]) {
    ...
    int costs_size = lws * sizeof(int);
    int *costs = malloc(costs_size);
    cl_mem d_costs = clCreateBuffer(info.context, CL_MEM_WRITE_ONLY | CL_MEM_USE_HOST_PTR, costs_size, costs, &err);
    ocl_check(err, "create d_costs");

    kernel_evt = kernel(info.queue, info.kernel, lws, d_adj, d_costs, v, totalPermutations);
